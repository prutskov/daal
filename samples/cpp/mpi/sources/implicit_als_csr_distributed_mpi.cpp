...
template<typename T> void gather(const ByteBuffer &nodeResults, T *result);
void gatherItems(const ByteBuffer &nodeResults);
template<typename T> void all2all(ByteBuffer *nodeResults, KeyValueDataCollectionPtr result);
...
void all2all(ByteBuffer *nodeResults, KeyValueDataCollectionPtr result)
{
    ...
    MPI_Alltoallv(&serializedSendData[0], perNodeArchLengths, sdispls, MPI_CHAR,
                  &serializedRecvData[0], perNodeArchLengthsRecv, rdispls, MPI_CHAR, MPI_COMM_WORLD);
